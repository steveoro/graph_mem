...
## Architecture
- multi-agent support: add a session layer to support multiple agent session at the same time, storing possibly the session ID and the latest context set for the session; the context can be set or cleared out, but it will remain associated to the same session allowing scoped memory nodes searches
